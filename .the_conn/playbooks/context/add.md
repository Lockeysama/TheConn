...
**当 Step 0 检测到已有相关 Context，用户选择"更新"时**：

```markdown
## 🔗 Context 更新衔接追踪

**当前工作流**: context/add → context/update

| Step | Playbook       | 状态 | 输出 | 备注        |
| ---- | -------------- | ---- | ---- | ----------- |
| 1    | context/update | ⏳    | -    | 准备更新... |
...
**衔接说明**：
- Step 0 检测到重复 Context → 询问用户
- 用户选择"更新"→ 自动调用 `@playbooks/context/update.md`
- 用户选择"创建新"→ 继续 Step 1（不衔接）
- 用户选择"取消"→ 结束流程（不衔接）
