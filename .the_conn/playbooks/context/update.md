...
本 Playbook **不依赖**以下规范：
- ❌ `test_strategy_rules.md` - Context 更新不涉及测试策略决策
- ❌ `bdd_language_rules.md` - Context 更新不涉及 BDD

**原因**: context/update 只负责更新 Context 文档，不涉及测试策略规划
...
| 类别         | 部分                                                | 说明                     |
| --- | --- | --- |
| **可以修改** | Frontmatter中的`updated`字段                        | 更新为当前日期           |
|              | Frontmatter中的`tags`字段                           | 可以添加或删除标签       |
|              | 文档正文                                            | 根据变更内容更新         |
|              | 新增章节                                            | Global Context可以添加   |
| **必须保持** | Frontmatter中的`type`,`scope`,`title`,`created`字段 | 不能修改                 |
|              | 文档的核心结构                                      | 保持章节组织的一致性     |
|              | Global Context文件数量                              | 不创建新的文件           |
...
| --- | --- |
| **精确定位**             | 明确要修改的章节或段落                                       |
| **保持一致**             | 更新后的内容应与文档整体风格一致                             |
| **版本记录**             | 在文档末尾可添加变更历史（可选）                             |
| **更新日期**             | 必须更新 Frontmatter 中的 `updated` 字段                     |
| **Global Context 优先更新** | 需要添加公约类型的内容时，优先更新 4 个固定文件，而不是创建新文件 |
| **章节组织**             | 新增内容应作为新章节添加，保持文档结构清晰                   |
