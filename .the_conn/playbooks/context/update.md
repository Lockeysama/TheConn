...
| 检测项                  | 冲突条件                             | 处理方式                                       |
| ----------------------- | ------------------------------------ | ---------------------------------------------- |
| **Frontmatter 保护**    | 用户要求修改 type/scope/created 字段 | ❌ 拒绝，提示"该字段受保护，参考 base_rules.md" |
| **Global Context 数量** | 用户要求创建第 5 个 Global Context   | ❌ 拒绝，提示"应添加章节到现有 4 个文件之一"    |
| **章节重名**            | 新增章节与现有章节名称重复           | ⚠️ 警告，询问用户: 合并/重命名/覆盖             |
