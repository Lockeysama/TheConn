...
- **不同框架的中文关键词可能不同**，AI 必须先分析框架的具体语法，再生成 BDD 场景
...
功能/Feature: {Feature 名称完整流程}

  场景/Scenario: {端到端场景1 - Happy Path}
    假如/Given {用户初始状态}
    当/When {执行完整操作流程}
    那么/Then {最终业务结果}
    并且/And {系统状态验证}

  场景/Scenario: {端到端场景2 - 异常处理}
    假如/Given {某个前置条件}
    当/When {触发异常情况}
    那么/Then {系统正确处理}
    并且/And {用户收到明确反馈}

  场景/Scenario: {端到端场景3 - 数据一致性}
    假如/Given {初始数据状态}
    当/When {跨模块操作}
    那么/Then {数据保持一致}
...
功能/Feature: {Epic 名称端到端用户旅程}

  场景/Scenario: 完整用户旅程 - Happy Path
    假如/Given {用户从零开始}
    当/When {执行跨 Feature 的完整流程}
    那么/Then {达成 Epic 的业务目标}
    并且/And {所有关键指标满足}

  场景/Scenario: 跨模块异常处理
    假如/Given {某个 Feature 出现异常}
    当/When {用户继续操作}
    那么/Then {系统优雅降级}
    并且/And {用户体验不受重大影响}
