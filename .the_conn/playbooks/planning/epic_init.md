# Epic 初始化指南

为新的 Epic 初始化基础设施，包括目录结构、上下文目录和初始 README。

## ⚠️ 重要：遵守基础公约

**本 Playbook 严格遵守 `@rules/base_rules.md` 中定义的所有基础公约。**

**📋 规范引用**：

本 Playbook 依赖以下规范文件（AI 必须先加载）：
- **基础公约**: `@rules/base_rules.md` - 禁止事项、文件路径约定、质量标准

本 Playbook **不依赖**以下规范：
- ❌ `test_strategy_rules.md` - Epic 初始化不涉及测试策略
- ❌ `bdd_language_rules.md` - Epic 初始化不涉及 BDD

**原因**: epic_init 只负责创建基础设施（目录和初始 README），不涉及测试规划

## 本 Playbook 的工作范围

**专注于**：初始化新 Epic 的基础设施

**使用说明**：
- **通常情况**：本 Playbook 由 `epic_planning.md` 自动调用，用户无需直接使用
- **直接使用场景**：仅需创建目录结构而不生成规划内容时，使用 `tc plan epic-init <名称>`
- **推荐方式**：直接使用 `tc epic <名称>`，会自动处理初始化和规划生成

**包括**：
1. 自动识别下一个可用的 Epic 编号
2. 创建 Epic 目录结构
3. 创建 Epic 上下文目录
4. 生成初始 Epic README.md 文件

**不包括**：
- 详细的 Epic 规划内容（使用 `@playbooks/planning/epic_planning.md`）
- Feature 和 Story 的生成

---

## 命名与格式规范

### ID 命名规则

- **格式**: `EPIC-{序号}`
- **序号**: 全局唯一，两位数字，从 01 开始
- **示例**: `EPIC-01`, `EPIC-02`, `EPIC-10`

### 目录命名规则

- **格式**: `EPIC-{序号}_{PascalCaseName}`
- **PascalCase**: 每个单词首字母大写，无分隔符
- **示例**: `EPIC-01_Base_Init`, `EPIC-02_UserManagement`

### 自动编号逻辑

1. **扫描现有 Epic**：列出 `.the_conn/epics/` 目录下所有以 `EPIC-` 开头的目录
2. **提取编号**：从目录名中提取数字编号（如 `EPIC-01_Name` → `01`）
3. **计算下一个编号**：找到最大编号 + 1，格式化为两位数字
4. **示例**：
   - 现有: `EPIC-01`, `EPIC-02`, `EPIC-05`
   - 下一个: `EPIC-06`

---

## 执行步骤

```mermaid
graph TD
    A[Step 1: 自动识别Epic编号] --> B[Step 2: 解析用户输入并确认]
    B --> C[Step 3: 创建目录结构]
    C --> D[Step 4: 生成初始Epic README]
    D --> E[Step 5: 确认完成]
```

### 执行追踪表格

**AI 必须在执行过程中维护以下追踪表格**：

```markdown
## 🔄 Epic 初始化执行追踪

| Step | 内容                 | 状态 | 输出 | 备注   |
| ---- | -------------------- | ---- | ---- | ------ |
| 1    | 自动识别 Epic 编号   | ⏳    | -    | 待开始 |
| 2    | 解析用户输入并确认   | ⏳    | -    | 待开始 |
| 3    | 创建目录结构         | ⏳    | -    | 待开始 |
| 4    | 生成初始 Epic README | ⏳    | -    | 待开始 |
| 5    | 确认完成             | ⏳    | -    | 待开始 |

**图例**：✅ 已完成 | 🔄 进行中 | ⏳ 等待中 | ❌ 失败
```

**更新规则**：
- 进入某个 Step → 状态更新为 🔄
- 完成某个 Step → 状态更新为 ✅，填写输出
- Step 失败 → 状态更新为 ❌，记录失败原因

---

### Step 1: 自动识别 Epic 编号

| 操作     | 说明                                     |
| -------- | ---------------------------------------- |
| 扫描目录 | 使用 `list_dir` 列出 `.the_conn/epics/`  |
| 提取编号 | 从目录名中提取 `EPIC-XX`                 |
| 计算编号 | 最大编号 + 1，格式化为两位数字           |
| 示例     | 现有: EPIC-01, EPIC-02 → 下一个: EPIC-03 |

### Step 2: 解析用户输入并确认

**用户输入格式**: `@tc.md epic-init <Epic名称> [简单描述]`

| 输入示例                                              | 名称         | 描述                                 | 目录名                 |
| ----------------------------------------------------- | ------------ | ------------------------------------ | ---------------------- |
| `epic-init 用户管理系统 实现用户注册、登录和权限管理` | 用户管理系统 | 实现用户注册、登录和权限管理         | EPIC-03_UserManagement |
| `epic-init 数据分析模块`                              | 数据分析模块 | 数据分析模块（使用名称作为默认描述） | EPIC-03_DataAnalysis   |

**与用户确认**: 编号/名称/描述/目录名

### Step 3: 创建目录结构

```
.the_conn/
├── epics/
│   └── EPIC-XX_{PascalCaseName}/
│       ├── README.md
│       └── features/              # 空目录（.gitkeep）
└── context/
    └── epics/
        └── EPIC-XX/               # Epic 专属上下文（.gitkeep）
```

**操作**: 使用 `write` 工具创建 README.md（自动创建父目录） + 创建 `.gitkeep` 占位文件

### Step 4: 生成初始 Epic README

**文件路径**: `.the_conn/epics/EPIC-XX_{PascalCaseName}/README.md`

```markdown
---
id: EPIC-XX
status: pending
created: yyyy-mm-dd
---

# EPIC-XX: {Epic 名称}

## 📋 概述
{用户输入的简单描述}

## 🎯 业务价值
待补充

## 📦 Features 概览

```mermaid
graph LR
    E[EPIC-XX] --> F1[FEAT-01<br/>待规划]
    E --> F2[FEAT-02<br/>待规划]
    E --> F3[FEAT-03<br/>待规划]
    
    F1 --> S1[Stories: 0/0]
    F2 --> S2[Stories: 0/0]
    F3 --> S3[Stories: 0/0]
```

### Features 列表

| ID      | Feature名称 | 状态    | Stories | 完成度 |
| ------- | ----------- | ------- | ------- | ------ |
| FEAT-01 | 待规划      | pending | 0/0     | 0%     |
| FEAT-02 | 待规划      | pending | 0/0     | 0%     |
| FEAT-03 | 待规划      | pending | 0/0     | 0%     |

待规划 - 使用 `tc plan feature` 或 `tc plan breakdown` 生成

## 📊 范围

### 包含
- 待补充

### 不包含
- 待补充

## 🎯 关键指标

| 指标        | 目标值 | 当前值 | 状态 |
| ----------- | ------ | ------ | ---- |
| 完成度      | 100%   | 0%     | ⏳    |
| Stories完成 | X个    | 0个    | ⏳    |
| 代码覆盖率  | ≥80%   | -      | ⏳    |

## 🧪 测试里程碑

```mermaid
graph LR
    M1[单元测试] --> M2[集成测试]
    M2 --> M3[E2E测试]
    M3 --> M4[发布]
```

待规划 - 根据 Features 确定后补充

## 📈 进度追踪

### 整体进度

| 阶段     | 状态 | 开始日期 | 完成日期 |
| -------- | ---- | -------- | -------- |
| 需求评审 | ⏳    | -        | -        |
| 设计完成 | ⏳    | -        | -        |
| 开发完成 | ⏳    | -        | -        |
| 测试完成 | ⏳    | -        | -        |
| 发布上线 | ⏳    | -        | -        |

### Features 进度

```mermaid
gantt
    title Features 开发时间线
    dateFormat  YYYY-MM-DD
    section FEAT-01
    待规划           :2025-01-01, 1d
    section FEAT-02
    待规划           :2025-01-01, 1d
    section FEAT-03
    待规划           :2025-01-01, 1d
```

---

## 📂 相关上下文
Epic 专属上下文位于: `.the_conn/context/epics/EPIC-XX/`

使用 `tc ctx add` 或提取命令添加上下文文档。
```

**字段说明**: `id`(Epic编号) / `status`(pending/done) / `created`(yyyy-mm-dd) / 其他字段保留为"待补充"

### Step 5: 确认完成

```
✅ Epic 初始化完成！

📁 创建的目录结构:
   ├── .the_conn/epics/EPIC-XX_{PascalCaseName}/
   │   ├── README.md
   │   └── features/
   └── .the_conn/context/epics/EPIC-XX/

📝 下一步建议:
   1. 使用 @tc.md plan review 来讨论并生成技术方案
```

---

## 使用示例

| 场景                 | 用户输入                                              | 名称         | 描述                         | 目录                   | 说明                            |
| -------------------- | ----------------------------------------------------- | ------------ | ---------------------------- | ---------------------- | ------------------------------- |
| 第一个Epic（带描述） | `epic-init 用户管理系统 实现用户注册、登录和权限管理` | 用户管理系统 | 实现用户注册、登录和权限管理 | EPIC-01_UserManagement | 未发现现有Epic，编号从01开始    |
| 后续Epic（仅名称）   | `epic-init 数据分析模块`                              | 数据分析模块 | 数据分析模块                 | EPIC-03_DataAnalysis   | 已有EPIC-01/02，下一个为EPIC-03 |

---

## 注意事项

| 项目       | 说明                                       |
| ---------- | ------------------------------------------ |
| 编号唯一性 | 系统自动确保不重复                         |
| 命名规范   | 自动转换为PascalCase用于目录名             |
| 极简内容   | 初始README仅包含用户描述，其他字段"待补充" |
| 描述处理   | 有描述用描述，无描述用名称                 |
| 上下文目录 | 创建空目录，等待后续添加文档               |
| Git友好    | 使用`.gitkeep`确保空目录可跟踪             |

---

## 与其他 Playbook 的配合

- **epic_planning.md**: 用于生成详细的 Epic 规划内容
- **requirements_breakdown.md**: 批量生成 Epic/Feature/Story
- **context/extract.md**: 提取 Epic 专属上下文
- **feature_planning.md**: 在 Epic 下创建 Features

**推荐工作流**：

```bash
# 1. 初始化 Epic 基础设施
@tc.md epic-init {Epic名称} {简单描述}

# 2. 讨论并生成技术方案
@tc.md plan review

# 3. 完善 Epic 规划（可选）
@tc.md epic {Epic名称}  # 使用 epic_planning.md 生成详细内容

# 4. 创建 Features
@tc.md feature {Feature名称}

# 5. 提取上下文
@tc.md ctx extract
```

---

现在，请提供 Epic 名称，我将为你初始化新的 Epic 基础设施。

