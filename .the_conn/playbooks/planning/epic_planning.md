...
|---------|--------------|----------|
| **限界上下文数量** | 1个（单体应用） | 2个以上（分布式） |
| **核心域标识** | 无需标识 | 需要标识（核心域🔴/支撑域🟡/通用域🟢） |
| **DDD 需求** | 不需要战略/战术设计 | 需要领域驱动设计 |
| **上下文映射** | 无需映射 | 需要映射关系（Customer-Supplier/ACL等） |
| **典型场景** | 技术项目、单服务CRUD | 跨服务、核心域重构、复杂业务规则 |
...
- **Standard Mode**: 使用标准格式（业务价值 + 范围 + 指标）
- **Pro Mode**: 使用完整格式（+ 限界上下文说明 + 上下文映射关系）
...
**AI 必须在生成 Epic 文档后执行以下检查**：

### 基础检查
- [ ] **文件路径正确**: `.the_conn/epics/EPIC-{序号}_{PascalCaseName}/README.md`
- [ ] **ID 格式正确**: EPIC-XX（两位数字）
- [ ] **目录命名规范**: PascalCase，无空格
- [ ] **Frontmatter 完整**: id / status / created / depends_on

### 内容检查
- [ ] **模式正确**: Standard Mode / Pro Mode（根据判定结果）
- [ ] **业务价值明确**: 一句话说明核心价值
- [ ] **范围边界清晰**: 明确包含和不包含的内容
- [ ] **关键指标可衡量**: 有具体数值或百分比

### Pro Mode 额外检查（如适用）
- [ ] **限界上下文说明完整**: 上下文名称 / 核心域标识 / 职责 / 边界
- [ ] **上下文映射关系明确**: 关系类型 / 映射模式 / 说明
- [ ] **通信模式定义**: 同步调用 / 异步事件
- [ ] **参考文档链接**: Review Phase 1.5 / ADR-XXX

### 测试里程碑检查
- [ ] **Feature 级测试说明**: 已引用各 Feature 规划
- [ ] **Epic E2E 测试**: 如需要，已规划 STORY-999
- [ ] **性能测试**: 如需要，已规划 STORY-997
- [ ] **测试理由清晰**: 说明为什么需要/不需要

### 依赖关系检查
- [ ] **依赖关系明确**: depends_on 字段正确
- [ ] **开发顺序合理**: 建议顺序符合依赖逻辑
- [ ] **并行机会识别**: 无依赖的部分已标识
- [ ] **风险提示完整**: 已列出关键风险

### Epic 数量异常检查
- [ ] **如 Epic > 5**: 考虑是否拆分过细，建议合并
- [ ] **如 Epic = 1 且 Feature > 6**: 考虑是否需要拆分 Epic
- [ ] **如所有 Epic 都很小**: 考虑粒度是否合理
