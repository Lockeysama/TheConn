...
生成 Feature 时，AI 按以下判断树决定是否需要 E2E 测试 Story：
...
```
Feature 复杂度 = Story 数量影响 + 平均 Story 复杂度 + 集成复杂度

其中：
- Story 数量影响 = Story 数量 × 0.5（上限 3.0）
- 平均 Story 复杂度 = Σ(Story 复杂度) / Story 数量
- 集成复杂度 = 低(1.0) / 中(2.0) / 高(3.0)
```

**集成复杂度判断**：
- **低(1.0)**：单一模块，依赖少
- **中(2.0)**：2-3 个模块集成，少量外部依赖
- **高(3.0)**：≥4 个模块集成，多个外部依赖

**评分示例**：

```
FEAT-01: 用户认证
- Story 数量：4 个 → 4 × 0.5 = 2.0
- 平均复杂度：(4.0 + 5.0 + 6.0 + 4.5) / 4 = 4.875
- 集成复杂度：高（数据库 + 缓存 + API）= 3.0
- Feature 复杂度：2.0 + 4.875 + 3.0 = 9.875 → 高复杂度

FEAT-02: 配置管理
- Story 数量：2 个 → 2 × 0.5 = 1.0
- 平均复杂度：(2.0 + 2.5) / 2 = 2.25
- 集成复杂度：低（单一模块）= 1.0
- Feature 复杂度：1.0 + 2.25 + 1.0 = 4.25 → 中等复杂度
```
