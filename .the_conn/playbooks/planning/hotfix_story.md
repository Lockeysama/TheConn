...
- [ ] **联动覆盖**: 已确认配套修改（如数据变动是否同步改了API/Migration）
...
- [ ] 数据模型/配置变更 -> 检查: Migration, API Schema, Service Layer
- [ ] 接口变更 -> 检查: 前端调用, API 文档, 生成的 Client
...
5. **完整的小闭环**: Hotfix 虽小，但必须包含完整的修改链路（如：改配置必改文档，改模型必改API）。**不要为了降低复杂度评分而阉割必要的配套修改。**
