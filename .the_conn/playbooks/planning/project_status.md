# 项目状态查看指南

自动生成项目当前进度报告，帮助快速了解项目整体状态。

## ⚠️ 重要：遵守基础公约

**本 Playbook 严格遵守 `@rules/base_rules.md` 中定义的所有基础公约。**

**📋 规范引用**：

本 Playbook 依赖以下规范文件（AI 必须先加载）：
- **基础公约**: `@rules/base_rules.md` - 禁止事项、文件路径约定、质量标准

本 Playbook **不依赖**以下规范：
- ❌ `test_strategy_rules.md` - 项目状态查看不涉及测试策略决策
- ❌ `bdd_language_rules.md` - 项目状态查看不涉及 BDD

**原因**: project_status 只负责生成进度报告，不涉及测试策略规划

## 本 Playbook 的工作范围

**专注于**：

- ✅ **生成报告**：创建项目进度报告
- ✅ **分析数据**：统计任务完成情况

**重要**：此 Playbook 是只读的，不修改任何文档。

---

## 使用场景

- 团队站会前快速了解进度
- 向干系人汇报项目状态
- 识别阻塞和风险
- 规划下一阶段工作

---

## 输入要求

需要访问：

- `.the_conn/epics/` 下的所有 Epic/Feature/Story 文件
- Story 文件的 Frontmatter（特别是 `status` 和 `depends_on` 字段）

---

## 输出格式（精简模板）

```markdown
# 项目状态报告

**生成时间**: {yyyy-mm-dd HH:mm} | **范围**: 所有 Epic

---

## 📊 总体概览

### 项目统计

| 指标   | Epic | Feature | Story | 完成率 |
| ------ | ---- | ------- | ----- | ------ |
| 总数   | {N}  | {N}     | {N}   | -      |
| 已完成 | {N}  | {N}     | {N}   | {X}%   |
| 进行中 | {N}  | {N}     | {N}   | {Y}%   |
| 待开始 | {N}  | {N}     | {N}   | {Z}%   |

### 整体进度可视化

```mermaid
pie title Story 状态分布
    "已完成" : {完成数量}
    "进行中" : {进行中数量}
    "待开始" : {待开始数量}
```

---

## 🎯 Epic 进度

### EPIC-01: {名称}

**状态**: {进行中/已完成/未开始} | **进度**: {X}% ({已完成}/{总数} Stories)

#### 进度条

```
[████████░░] 80%
```

#### Features 详情

| Feature | 名称   | Stories完成 | 进度 | 状态     |
| ------- | ------ | ----------- | ---- | -------- |
| FEAT-01 | {名称} | 5/5         | 100% | ✅ 已完成 |
| FEAT-02 | {名称} | 2/5         | 40%  | 🚧 进行中 |
| FEAT-03 | {名称} | 0/3         | 0%   | ⏸️ 未开始 |

#### Stories 概览

```mermaid
graph LR
    S01[STORY-01✅] --> S02[STORY-02✅]
    S02 --> S03[STORY-03🚧]
    S03 --> S04[STORY-04⏸️]
```

---

## 🚧 当前进行中 / ⏸️ 待开始（Top 5）

| Story ID | 名称   | Epic    | Feature | 依赖状态 | 可开始时间 |
| -------- | ------ | ------- | ------- | -------- | ---------- |
| STORY-03 | {名称} | EPIC-01 | FEAT-02 | -        | -          |

---

## 🚨 阻塞项

### 阻塞 Story

| Story ID | 名称   | 被阻塞原因 | 依赖的Story | 影响范围              |
| -------- | ------ | ---------- | ----------- | --------------------- |
| STORY-08 | {名称} | 依赖未完成 | STORY-05    | 阻塞STORY-09,STORY-10 |

### 依赖链路（Mermaid可选）

```mermaid
graph TD
  STORY-05[STORY-05 进行中] --> STORY-08[STORY-08 被阻塞]
  STORY-08 --> STORY-09[STORY-09 被阻塞]
```

**建议**: 优先完成关键路径Story

---

## 📈 进度趋势

### 按 Epic

```
EPIC-01: ████████░░ 80%
EPIC-02: ███░░░░░░░ 30%
EPIC-03: ░░░░░░░░░░ 0%
```

### 按 Feature

```
FEAT-01: ██████████ 100%
FEAT-02: ████░░░░░░ 40%
FEAT-03: ██░░░░░░░░ 20%
FEAT-04: ░░░░░░░░░░ 0%
```

---

## ⚠️ 风险识别

### 高风险项

1. **EPIC-02 进度滞后**
   - 现状: 仅完成 30%
   - 风险: 可能影响整体交付
   - 建议: 增加资源或调整范围

2. **STORY-05 成为关键路径**
   - 现状: 阻塞 3 个后续 Story
   - 风险: 一旦延期，影响连锁
   - 建议: 优先完成，密切跟踪

---

## 📅 里程碑

| 里程碑    | 目标日期 | 关联 Epic | 状态     |
| --------- | -------- | --------- | -------- |
| {里程碑1} | {日期}   | EPIC-01   | ✅ 已达成 |
| {里程碑2} | {日期}   | EPIC-02   | 🚧 进行中 |
| {里程碑3} | {日期}   | EPIC-03   | ⏸️ 未开始 |

---

## 💡 下一步建议

### 本周重点

1. **完成 STORY-05**（解除阻塞）
2. **启动 STORY-04**（无依赖，可并行）
3. **评审 EPIC-02**（进度滞后，需调整）

### 需要决策

- [ ] EPIC-02 是否需要调整范围？
- [ ] STORY-03 是否需要增加资源？
- [ ] EPIC-03 何时启动？

---

## 📊 数据明细

### Story 状态分布

| 状态    | 数量 | 占比 |
| ------- | ---- | ---- |
| done    | {N}  | {X}% |
| pending | {N}  | {X}% |

### Epic 分布

| Epic    | Stories | 已完成 | 进行中 | 待开始 |
| ------- | ------- | ------ | ------ | ------ |
| EPIC-01 | {N}     | {N}    | {N}    | {N}    |
| EPIC-02 | {N}     | {N}    | {N}    | {N}    |

---

## 📝 备注 / 图例说明

| 符号 | 含义     | 说明               |
| ---- | -------- | ------------------ |
| ✅    | 已完成   | status:done        |
| 🚧    | 进行中   | pending+有Task     |
| ⏸️    | 待开始   | pending+依赖已满足 |
| 🚨    | 被阻塞   | pending+依赖未满足 |
| ⚠️    | 需要关注 | -                  |

- 本报告基于 Frontmatter 的 `status` 字段生成
- "进行中" 指当前有 Task 正在执行的 Story
- 阻塞识别基于 `depends_on` 字段
- 更新状态使用 @prompts/execution/story_sync.md
```

---

## 执行流程追踪

**AI 必须在执行过程中维护以下追踪表格**：

```markdown
## 🔄 项目状态生成执行追踪

| Phase | 内容               | 状态 | 输出 | 备注   |
| ----- | ------------------ | ---- | ---- | ------ |
| 1     | 扫描 Story 文件    | ⏳    | -    | 待开始 |
| 2     | 计算统计数据       | ⏳    | -    | 待开始 |
| 3     | 识别阻塞与依赖链路 | ⏳    | -    | 待开始 |
| 4     | 生成报告           | ⏳    | -    | 待开始 |

**图例**：✅ 已完成 | 🔄 进行中 | ⏳ 等待中 | ❌ 失败
```

---

## 生成逻辑

| 步骤          | 操作                                                                                                                   | 说明                                        |
| ------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- |
| 1. 扫描Story  | 遍历 `.the_conn/epics/**/stories/*.md`                                                                                 | 提取: id/status/epic/feature/depends_on     |
| 2. 计算统计   | 总数=所有文件数<br/>已完成=status:done数量<br/>待开始=status:pending且依赖已满足<br/>进行中=status:pending且当前有Task | 基于ai_workspace目录判断进行中              |
| 3. 识别阻塞   | 对每个pending Story检查depends_on                                                                                      | 任何依赖Story的status≠done → 标记为"被阻塞" |
| 4. 生成进度条 | Epic进度=(已完成Story/总Story)×100%<br/>Feature进度同理                                                                | -                                           |

### "进行中" Story 判断逻辑

**优先策略**（按顺序尝试）：

1. **检查 ai_workspace 目录**:
   ```bash
   find .the_conn/ai_workspace -name "TASK-*_STORY-*" -type d
   ```
   如果目录存在 → 对应 Story 为"进行中"

2. **降级策略**（ai_workspace 不可访问时）:
   - 检查 Story 的 `updated` 字段（最近 7 天内修改 + status:pending）
   - 输出警告: "⚠️ 无法访问 ai_workspace，基于 updated 字段推断进行中 Story"

---

## 使用示例

```

@.the_conn/epics/ @prompts/planning/project_status.md 生成项目状态报告

```

或指定特定 Epic：

```

@.the_conn/epics/EPIC-01_Base_Init/ @prompts/planning/project_status.md 生成 EPIC-01 状态报告

```

---

## 注意事项

1. **依赖准确性**: 报告准确性取决于 `depends_on` 字段是否正确维护
2. **状态时效性**: 确保通过 `story_sync.md` 及时更新 Story 状态
3. **进行中判断**: 如果无法访问 `ai_workspace`，"进行中"可能不准确
4. **手动验证**: 报告仅供参考，重要决策需人工验证

---

现在，请指定要查看的范围（所有 Epic 或特定 Epic），我将生成状态报告。
