...
**AI 必须确认以下项目后才能进入 Step 2**：

- [ ] 已提取关键信息：父 Story ID (如有) / 变更关键词 / 功能模块 / 技术关键词
- [ ] 已调用 `@playbooks/context/search.md` (可返回空结果)
- [ ] 已更新追踪表格：Step 1.1 和 1.2 状态为 ✅
- [ ] 已记录 Context 搜索结果 (如有)
...
**模式匹配规则**:
- 包含 "不工作" / "不正常" / "无法" / "不能" → bug_fix
- 包含 "空指针" / "内存泄漏" / "死锁" → bug_fix
- 包含 "返回错误" / "数据错误" / "不一致" → bug_fix
...
```
增加、添加、优化、改进、调整
add、improve、optimize、enhance、refactor
```
...
**AI 必须确认以下项目后才能进入 Step 3**：

- [ ] 已判断变更类型：bug_fix / hotfix / 需用户确认
- [ ] 如有 STORY-XX，已读取验收标准并检查是否违反契约
- [ ] 已应用关键词库判断（Bug 关键词优先）
- [ ] 已更新追踪表格：Step 2 状态为 ✅
- [ ] 已记录判断理由
...
**AI 必须确认以下项目后才能进入 Step 4**：

- [ ] 已确定归属关系：父 Story ID / Epic ID / Feature ID
- [ ] 已确定新 Story ID 格式 (STORY-XX.Y 或 STORY-XX)
- [ ] 如需用户选择，已列出选项并获得确认
- [ ] 已更新追踪表格：Step 3 状态为 ✅
- [ ] 已记录推断路径 (情况 A/B/C 及优先级)
...
**AI 必须确认以下项目后才能进入 Step 5 (自动衔接)**：

- [ ] 已输出分析报告 (包含类型/理由/归属/新ID/路由)
- [ ] 已获得用户确认 "继续? [y/n]"
- [ ] 已加载目标 Playbook (bug_fix_story.md / hotfix_story.md)
- [ ] 已更新追踪表格：Step 4 状态为 ✅
- [ ] 已准备好传递参数
...
Quick Change 通常是快速任务，但以下情况可能需要多轮交互：
1. **类型判断不明确**：同时包含 Bug 和 Hotfix 关键词
2. **归属推断困难**：无明确线索，需多次尝试
3. **用户反复修改**：确认后又想调整
...
| 检查项         | 状态                 | 说明                      |
| -------------- | -------------------- | ------------------------- |
| 类型判断完成   | 是/否                | bug_fix / hotfix          |
| 归属关系确定   | 是/否                | 父 Story / Epic / Feature |
| 用户确认状态   | 待确认/已确认/已拒绝 | 最近一次反馈              |
| 最近获得新信息 | 是/否                | 类型/归属/用户偏好        |

**触发的收敛条件**：
- ✅ 类型和归属都已确定 + 用户确认 → 继续执行
- ✅ 连续 2 次推断失败 → 强制收敛，列出完整选项
- ✅ 用户表达困惑（"不确定"/"再想想"）→ 提供更多背景
- ✅ 陷入循环（重复询问相同问题）→ 切换推断策略
- ⏸️ 无触发，继续交互
```
...
| 触发条件             | 检测方式                              | AI 行动                             | 输出                 |
| -------------------- | ------------------------------------- | ----------------------------------- | -------------------- |
| **类型和归属都确定** | 类型 + 归属都有明确值 + 用户未拒绝    | 输出分析报告<br/>请用户确认         | 分析报告 + 确认提示  |
| **连续推断失败**     | 连续 2 次推断失败（未找到相关 Story） | 强制收敛<br/>列出完整选项供直接选择 | 完整选项列表         |
| **用户表达困惑**     | 关键词："不确定"/"再想想"/"有点乱"    | 提供更多背景信息<br/>展示示例       | 背景说明 + 示例      |
| **陷入循环**         | 同一问题被询问 2 次                   | 切换推断策略<br/>或提供完整选项     | 策略切换 + 选项      |
| **用户明确拒绝**     | 用户拒绝推断的归属关系                | 切换下一个推断策略<br/>直至完整列表 | 下一策略 or 完整列表 |
...
graph TD
    A[每次交互后] --> B{检查触发条件}
    B -->|类型+归属都确定| C1[✅ 输出分析报告<br/>请用户确认]
    B -->|连续2次推断失败| C2[⚠️ 强制收敛<br/>列出完整选项]
    B -->|用户表达困惑| C3[💡 提供背景信息<br/>展示示例]
    B -->|检测到循环| C4[🔄 切换推断策略<br/>或列完整选项]
    B -->|用户拒绝推断| C5[➡️ 下一推断策略<br/>直至完整列表]
    B -->|无触发条件| D[✅ 继续交互]
```

**强制收敛模板（连续 2 次推断失败时触发）**：

```markdown
⚠️ 推断策略未能成功识别归属关系，为提高效率，请直接选择：
...
| 选项 | Epic       | Feature | 说明       |
| ---- | ---------- | ------- | ---------- |
| 1    | EPIC-01    | FEAT-01 | 用户认证   |
| 2    | EPIC-02    | FEAT-03 | 订单管理   |
| 3    | 独立 Story | -       | 不属于现有 |
...
```
✓ 本轮是否获得新信息？（类型/归属/用户偏好）
✓ 用户是否明确表达困惑？（"不确定"/"再想想"）
✓ 是否陷入循环？（重复询问相同问题）
✓ 推断失败次数？（连续 2 次 → 触发强制收敛）
✓ 是否触发任何收敛条件？
```
...
8. **基于触发条件**: 不依赖轮数判断，使用明确的触发条件进行决策收敛 🆕
