...
> 🔴 **AI 重要提示**：
> - 下表中的 Playbook 路径是**命令路由映射**，不是待加载文件列表
> - 只加载用户命令对应的那**一个** Playbook，不要遍历表格加载所有路径
> - 例如：用户输入 `tc quick` → 只加载 `planning/quick_router.md`
...
**AI 在加载第 3 层规范前必须执行去重检查**：

```markdown
检查步骤：
1. 维护"已加载文件列表"
2. 准备加载文件 X 时 → 检查是否已在列表中
3. 如果是 → 跳过并记录"✓ 已加载，跳过"
4. 如果否 → 加载并添加到列表
```
