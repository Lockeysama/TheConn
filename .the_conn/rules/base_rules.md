...
### 10.9 状态持久化 (State Persistence) 🔴

**为防止 LLM 在长对话中丢失上下文，AI 必须遵守以下规则**：

**规则：每次回复的末尾，必须附带当前的 [状态快照]**。

即使 Playbook 未明确要求，只要是**多轮对话且涉及状态变更**的任务，都必须输出。

**快照格式（Minimal Markdown）**：

```markdown
<!-- State Snapshot -->
> **Status**: 🔄 Step 3/5 | **Context**: 3 Files Loaded
> **Next**: 执行 Step 4
```

**目的**：确保即使 Context 被截断，AI 也能通过读取最后一条消息快速恢复状态。
